...
**Verified on v0.2.23** (local SQLite, free high port, no live MinIO): register → agent token → MCP `whoami` / `list_drives` → create/claim/complete job → `ensure_mounted_hint` → offline Qiniu `object_presign_get` (`method=qiniu_download`).
...
curl -sS --noproxy '*' "$API/healthz"   # expect "version":"0.2.23" (or your build)
