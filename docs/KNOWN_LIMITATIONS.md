...
- Job webhook（可选）：durable outbox `job_webhook_outbox` + worker；envelope `{event_id,event,occurred_at,job}`；HMAC optional；at-least-once（按 event_id 去重）；失败退避后 `dead`；无 admin UI 重放 dead 行。
