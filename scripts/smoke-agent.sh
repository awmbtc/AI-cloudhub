...
HTTP_ADDR=":${API_PORT}" AI_CLOUDHUB_DB="$DB" JWT_SECRET="${JWT_SECRET:-agent-smoke-jwt-secretxx}" \
  ./.bin/api > /tmp/aihub-agent-api.log 2>&1 &
APID=$!
...
for i in $(seq 1 50); do
  ...
  if ! kill -0 "$APID" 2>/dev/null; then
    ...
    cat /tmp/aihub-agent-api.log >&2 || true
    exit 1
  fi
  sleep 0.1
done
