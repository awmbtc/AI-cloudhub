...
func (p *Postgres) AppendAudit(e *AuditEvent) error {
	_, err := p.db.Exec(
		`INSERT INTO audit_events (id, user_id, action, target, detail, created_at) VALUES ($1,$2,$3,$4,$5,$6)`,
		e.ID, e.UserID, e.Action, e.Target, e.Detail, e.CreatedAt.UTC(),
	)
	return err
}
...
func (p *Postgres) ListAudit(f AuditFilter) ([]*AuditEvent, error) {
	...
	for rows.Next() {
		var e AuditEvent
		if err := rows.Scan(&e.ID, &e.UserID, &e.Action, &e.Target, &e.Detail, &e.CreatedAt); err != nil {
			return nil, err
		}
		out = append(out, &e)
	}
